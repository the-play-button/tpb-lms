 ...
 * 
 * Authentication:
 * - Uses native Bearer tokens (bastion_xxx) generated by TPB Bastion
 * - Token is stored in env.BASTION_TOKEN (via wrangler secret or .devcontainer/.env)
 * 
 * Usage:
...
export class VaultClient {
   ...
   * @param {object} env - Worker env with BASTION_TOKEN
   */
  ...
  /**
   * Auth abstraction - Native Bearer token (bastion_xxx)
   * 
   * Uses BASTION_TOKEN from environment (set via wrangler secret or .devcontainer/.env)
   * 
   * @returns {Promise<object>} - Headers object for authentication
   */
  async getAuthHeaders() {
    if (!this.env.BASTION_TOKEN) {
      throw new Error('BASTION_TOKEN not configured. Run: wrangler secret put BASTION_TOKEN');
    }
    
    return {
      'Authorization': `Bearer ${this.env.BASTION_TOKEN}`
    };
  }
