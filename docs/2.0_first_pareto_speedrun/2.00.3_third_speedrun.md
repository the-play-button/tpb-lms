...
```javascript
// Resolve role from email (BASTION pattern)
...
const BADGES = {
  none:   { icon: '⚪', label: 'Non commence', color: '#666' },
  bronze: { icon: '🥉', label: 'Bronze (25%)', color: '#cd7f32' },
  silver: { icon: '🥈', label: 'Silver (50%)', color: '#c0c0c0' },
  gold:   { icon: '🥇', label: 'Gold (75%)', color: '#ffd700' },
  master: { icon: '👑', label: 'Master (100%)', color: '#9b59b6' }
};
...
```javascript
1. GAP-1504 (10 min) → Organisation middleware/
2. GAP-1208 (20 min) → resolveRole()
3. GAP-1201 (15 min) → Roles dans userContext
4. GAP-604  (20 min) → v_admin_overview + endpoint
5. GAP-602  (15 min) → v_course_mastery enrichi
6. GAP-112  (45 min) → Mastery badges UI
7. GAP-101  (45 min) → Playback speed
8. GAP-1502 (30 min) → OpenAPI doc
```
